...
- R29 Europe PMC 两篇全文 HTTP500：PMC10463176、PMC6933872，报告正确披露且未当作非OA；3篇全文已取回。属于外部抓取失败，不据此宣称平台源码错误。
- R29 首次 /info 网络超时后自动恢复；未重新创建线程。
- SQL调试返回 data 有32行、计数字段却为0，使用实际行核对；原因未追踪，未发送外部问题单。
