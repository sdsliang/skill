...
- every chart product file passes a JS syntax check (extracted `<script>` parses; `const CHART = {` has a single `{`; closing `};` intact) before being written to `/workspace/visualizations/`;
