...
- 同试验时间轴无 lieflat 现成组件，按 lieflat **L11 Trend Lineage（事件序列生命史）** 语法延伸为本模板：圆点=事件节点（实心=披露/更新，空心=里程碑锚点）、圆点大小=证据成熟度（phase）、圆点悬浮于基线上方、数值**纵排多行**在圆点下方（value 支持 `\n` 手动分行 + 自动折行，各节点首行顶对齐；离开轴线，基线下方为日期）、标签底对齐贴节点、端部锚定（首左/末右）防溢出；无矩形/三角/刻线。
