...
出现在 FAIL 列表、③ 所有变异合起来覆盖清单里每一条 fail 级条目**。当前结果：

```
arm a: flipped 30/30   (14 个变异：改主终点数值、翻转安慰剂符号、抹掉试验登记号、篡改时点、
                        把 ref_1 指向不存在的引用、把标题里的药名写成另一个、把 2 条写成 5 条、
