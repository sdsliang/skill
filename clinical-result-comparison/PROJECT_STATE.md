...
> **追加（同日第六轮，待用户定级）**：用户提问「字段值基于摘要生成，PMC 能取到的能不能直接读全文」⇒ 实测成立：3 篇有 PMCID 的记录，全文数值 **334/334/249** 个，其中 **92%/88%/99% 不在摘要里**，`PFS`/`DoR`/`HR`/`TTR`/亚组/`Grade 3` 摘要根本不写。**但深度不对称**：会议摘要类（本切片 55.6%）外部永久只有摘要级，登记平台类库内已是全文级。给出 L1/L2/L3（建议 **L2：全文补齐库内空档 + 覆盖行标注来源深度**），**未改任何规则**，见台账 **O22** 与证据 JSON 的 `pmc_fulltext_information_gain`。
