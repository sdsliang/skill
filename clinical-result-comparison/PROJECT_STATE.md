...
- **当前状态：R30 已完成**。本轮数值事实账本/完整附表版本已原地更新 TS 并回读一致：prompt v1.6 57,656 B / SHA `115bac4e5295`；skill v1.0.7、20/20 文件一致；dist 93,090 B / SHA `dd4d83f871692bd4c70b98cbf6787af1b6f839426312aee017e75630d95a06bb`。固定三组 R30-1/2/3 均成功产出，分别覆盖 4/18/14 条；报告、引用、图表、实体锚点、固定路径和回执归档通过。R30-1 的支持文件断言曾受本地 `__pycache__` 污染，已清理；R30-2/R30-3 的 `no tool errors` 因平台运行时重复 `write_file` 已存在文件失败。原始证据保留，不能宣称全绿。详情见 `docs/toolsmith-verification-log.md`。


- **本轮新增数值完整性工作（本地待发布）**：新增 `scripts/render-facts.py` 和 `evals/numeric-integrity/test-render-facts.py`。事实账本将每个材料数字绑定到披露/研究/终点/治疗臂/人群/时间点/单位/统计量/源引文；渲染器复用 `[[fact:F001]]` / `[[result:F001]]`，校验 token、引用、源文件、精确引文和值绑定，并按披露生成完整字段附表。统一、跨试验、混合、同试验模板均要求一披露/一终点/一比较一行，正文减少重复数字。10 项测试全过；本地包 20 文件、92,491 B、SHA `ebb73e12aa9ea68d563a23bfd5db6c1b8fd5890d23661d902931bbadafa174a9`，pack/diff 检查通过。尚未发布或运行 R30+；历史 R27–R29 保留不变。



- **最新用户决策（测试 ID / 设计字段）**：MCP 设计字段来自登记平台，重点原文复核，冲突以原文为准并披露差异。ID 兼容仅在我们调用 TS 测试前以只读 SQL 解析，不进入 Skill；显式字段按指定字段，未指定按 `_id OR extra_esid`，保留映射和未解析项。见 [决策与证据](docs/test-input-resolution.md)。本批转换 86 个真实 `_id`、68 个未解析；未改运行资产、未发布。**撤回下文 A10 的“上下文误抽”归因**：用户 ES 的 Double-Blind / 双盲正确，MCP 开放是跨来源差异，不能归罪原始抽取。
