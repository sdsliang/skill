...
11. `references/chart-templates.md` before producing any chart — chart products are pure JSON and **reuse the upstream `chart-visualization-json` skill** (its protocol, templates, Zod schemas and CLI validator) instead of a local implementation
...
**Optional for large batches**: if the deployment exposes the `task` tool (subagents capability enabled) and there are roughly 20+ selected esids, you MAY delegate per-chunk extraction to subagents to keep context bounded, per `references/input-contract.md` (Large-batch delegation). This is a performance strategy pending real-environment verification — never treat it as mandatory, never skip an esid, and keep the default direct-pull protocol otherwise.
