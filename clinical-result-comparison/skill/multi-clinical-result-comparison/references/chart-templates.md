...
- 同试验时间轴无 lieflat 现成组件，按 lieflat **L11 Trend Lineage（事件序列生命史）** 语法延伸为本模板：圆点=事件节点（实心=披露/更新，空心=里程碑锚点）、圆点大小=证据成熟度（phase）、基线=时间轴即证据链（draw 动画）、数值横排在节点右侧（末节点在左侧防溢出）、标签/日期自动折行 + 端部锚定；无矩形/三角/刻线。
