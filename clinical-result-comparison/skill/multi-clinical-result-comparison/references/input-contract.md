...
**Study-design verification (registry fields versus the selected paper).** The MCP design fields `blinded`, `randomized`, `trial_control` and `positive_placebo_control` are based on registry information, not necessarily extracted from the selected paper. For every record, explicitly compare each available field with the selected study's Methods/design description in the deepest permitted original already retrieved (or its abstract when full text is unavailable). Apply this check to prose, comparison tables and chart labels before delivery; successful retrieval alone does not count as verification. Report the design stated in the paper when it differs, and disclose both descriptions next to the relevant claim, e.g. `原文：双盲；登记平台记录：开放{{ref_n}}`. Use this wording only for an observed difference, never as boilerplate.

Compare the same cohort, study period and design aspect: initial blinding is distinct from post-unblinding open-label treatment; blinded outcome assessment is distinct from participant blinding; references to other trials are not evidence about this study. A paper's double-blind description versus registry triple/quadruple masking can reflect role-count conventions. Preserve the paper's wording and the registry description, explain a role/version difference only when supported, and otherwise say the reason is unconfirmed; do not call it an extraction error merely from different labels. If the paper does not state an aspect, its absence is not a conflict: attribute any retained design value to the registry and mark that aspect as not confirmed in the available original. If no original is usable, disclose the verification gap rather than claiming a match. These checks do not authorize additional searches or expand retrieval budgets.
