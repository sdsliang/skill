...
6. **Call the platform tools by their exact argument names.** The shell tool is `execute` with its command in the `shell_command` argument; writing `command` instead fails **silently** (no output, no error message, one wasted step). When a file is written through the shell, use a quoted heredoc (`python - <<'PY' … PY`) so the shell does not interpolate braces in JS/JSON, and keep the directory-creating `mkdir -p` in the *same* `shell_command` as the write — **including when the write target is a shell redirection**, because the shell resolves `> /workspace/visualizations/x.json` before the script runs.
