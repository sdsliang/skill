...
> **证据范围：** 本报告仅整合所选结果材料。相同数据截止和分析的重复披露合并为同一证据状态，不计作独立验证。正文中的数字和关键事实均可追溯到具体来源，并由前端展示为行内上角标引用。若有选中的结果未能取回（后端未返回记录），在此逐条点名说明，且它们不占用引用编号、不出现于引用文件。**原文核对：** [已按原文复核的记录数/取回记录数 + 每条按来源类标注复核路径（src=1 PubMed→PMID、src=2/187 登记平台→注册号 API、src=37 会议→DOI、src=120 补录→DOI 或公司页、src=49 新闻稿=库内正文即通稿原文无需外部抓取、src=398 SEC 不可复核），以及未能复核的记录及其原因类（抓取受限 / 无登记号或 DOI / 该来源不公开）]。原文优先于库内加工字段，两者不一致时按 `原文 …；库内记录 …{{ref_n}}` 写明双值，不静默取一侧。
