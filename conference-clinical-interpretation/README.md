...
1. **带图表的 Markdown 解读报告** —— 会议全景、热点主题洞察、代表性证据、负面/终止信号、推荐关注概览、中国企业/中国创新药参与（可选）。
2. **推荐关注清单** —— 独立 CSV（`watchlist.csv`）；报告正文只给概览（Top 3-5 摘要 + CSV 引用），**不展开完整明细**，避免与 CSV 重复维护。

核心立场：**热度 ≠ 记录数**。记录曝光多不代表临床价值高；价值判断综合评价质量、样本量、阶段、终点证据强度与机制新颖度。

## 分析维度

参考医药媒体（世易医健/良医汇/丁香通等）对 ASCO 2026 的分析框架，结合数据可得性，支持以下维度（详见 `references/analysis-dimensions.md`）：

- 评价/结果方向、阶段/疾病阶段、癌种/适应症、治疗线数、终点类型、生物标志物、发布节奏（数据直接支持）。
- 药物技术路线/机制聚类（ADC/双抗/免疫/小分子）、中国企业/中国创新药参与（经药物画像 companies 近似判定）——部分支持，需标注局限。
- 报告级别（Oral/LBA/壁报）当前数据缺失，在「信息缺口与局限」中说明，不臆造。
...
│   │   ├── analysis-dimensions.md# 媒体分析维度参考与数据支持度对照
│   │   ├── watchlist.md          # 推荐关注清单评分与生成规则
│   │   └── drug-enrichment.md    # 药物画像增强（靶点/MOA/中国企业判定）
│   └── templates/
