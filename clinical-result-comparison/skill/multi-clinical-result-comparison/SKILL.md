...
10. `references/entity-inline-reference.md` for the v0.10 entity inline references (drug / company / trial registration number, only when their entity-ID metadata lines are present)

## Input

Each selected clinical result arrives as **one attached file** (`source-001.md`, `source-002.md`, …), one file per result, sent together as attachments in the user turn. Read every attached file from the workspace uploads directory. Each file contains backend-supplied citation metadata lines (`source_title`, `source_url`, `source_paper_release_time_str`) and the full text under the `## source_full_text` heading. When present, it also carries optional entity-ID metadata lines for inline references (`source_nct_id`, `source_drug_entities`, `source_company_entities`) — see `references/entity-inline-reference.md`. File order defines the `{{ref_n}}` marker assignment order; it is a presentation label, not clinical chronology. Follow `references/input-contract.md` for the exact file layout, limits, and reading protocol. Never print, quote, or transmit the files' raw metadata fields or filenames into the report body.
...
The supplied full text (the body of each attached source file) is the sole clinical evidence. Each file also carries backend-supplied citation metadata: `source_title`, `source_url`, and `source_paper_release_time_str`. Optional entity-ID metadata lines (`source_nct_id`, `source_drug_entities`, `source_company_entities`) are **display/label metadata, not clinical evidence** — they exist only to attach entity inline references to mentions already present in the report. Do not retrieve external facts or URLs, fill gaps from memory, or use technical correlation keys as evidence. Require at least two usable result files. Preserve unusable or incomplete items in the source inventory and explain their limitation if they affect the report.
...
During drafting, attach **entity inline references** per `references/entity-inline-reference.md`: whenever an entity-ID metadata line is present, wrap every mention of that drug (`entity:drug:`), company (`entity:company:`), and the registration number (`entity:trial:<source_nct_id>`) in the report with the `[name](entity:type:id)` form, next to its `{{ref_n}}` marker when present. Only use IDs explicitly given in the metadata lines; never fabricate IDs or query tools for them. Keep entity references out of `::visualization` chart files.
