...
Each selected clinical result is identified by its backend esid (the clinical-result ID, `clinical_result.extra_esid`). The frontend/user message supplies the selected esid list in user-selection order (that order defines the `{{ref_n}}` marker assignment). The Skill does **not** receive per-source full-text `.md` attachments; it pulls the detail records itself through the MCP tool `pharmcube-query-clinical-result-with-params` (see `references/input-contract.md`): pass `extra_esids: [...selected esids]` plus a **strict, minimal `selected_fields` allowlist** whose names are copied verbatim from the tool's own `selected_fields` parameter description (the runtime authority: it embeds the full `ALLOWED_FIELD_NAMES` / `ALLOWED_FIELDS` list; the repo keeps an offline mirror at `docs/params-tool-schema.md` for maintainers). Never invent a field name, and never assume a concept has a field just because the report needs it: if `ALLOWED_FIELD_NAMES` has no match, use the closest real field (e.g. `clinical_result.group_count` for group/arm counts) or drop the field — an invented name fails the whole pull. Keep `selected_fields` small to bound the returned payload (a single large response can exceed the context budget). The returned records map back to the supplied esids by `clinical_result.extra_esid`, and markers are assigned in user-selection order over the records that actually came back → `{{ref_1}}, {{ref_2}}, …`. One esid can come back as **several rows** (with `indication_name` requested the backend fans out on a disease-dimension join; those rows differ only in an injected `disease_id`): de-duplicate by `clinical_result.extra_esid` and treat them as **one** record — one marker, one citation entry, one entry in every count, evidence state and timeline event. An esid that returns no record gets **no** marker and **no** citation entry (never write a citation with an empty `title`): the params tool answers `ok: true` with an empty `data` array for an unknown or invisible esid, so confirm missing esids in **one** further batched call at most and never probe variant spellings or guessed ids. Follow `references/input-contract.md` for the exact field mapping, recommended `selected_fields`, response limits, and the unretrievable-item protocol. Never print, quote, or transmit raw field values, filenames, or tool internals into the report body.
...
The selected `selected_fields` that carry clinical content — `abstract_text`, `summary`, `study_results` (structured endpoint results), plus the design/arms context fields — are the **processed extracts** of the evidence for each record: the original source is the first priority for material clinical numbers and the pulled extract is the second, re-checked through the whitelisted route templates in `references/input-contract.md` (*Evidence source priority: the original source comes first*). **Read `abstract_text` first: for most classes the pulled body *is* the original** (measured 2026-09-17: journal abstracts match Europe PMC 0.992–1.000 after normalisation, conference abstracts match OpenAlex 0.991–0.999, press releases carry the wire dateline verbatim, registry JSON holds every decimal value the CT.gov API returns), so the default is to check the report against the pulled body with no external call. That section also maps the esid's middle segment (the ingestion source id: `1` PubMed, `2`/`187` ClinicalTrials.gov, `37` conference, `49` press release, `120` manual, `245`/`398`/unlisted unclassified) to what the original actually is per class — for `src=49` and the uuid legacy rows the pulled `abstract_text` **is** the release text; for the registration classes it is the registry's structured-results JSON and `R2` is the route; for `398` the original is not re-checkable and only the reason class gets recorded. **The one documented exception is `src=1`:** even with an abstract in hand, try `R6` for a `pmcid` and then `R7` for the **PMC full text** (56 of 100 sampled rows carry one; R7 measured 4/5 working, 63–99 KB of JATS full text), and when it returns, **the full text is the analysis surface** — depth decides how much of the record is available, never whether records are comparable (measured: 88–99 % of the full text's numerals appear nowhere in the abstract). Facts that exist only there are allowed when they carry their own 时点 / 分析集 / 人群 label, and such a record's citation `link` must then point at the full text (`https://pmc.ncbi.nlm.nih.gov/articles/{PMCID}/`, or the `…/{PMCID}/fullTextXML` source actually read) instead of the pulled link — the one exception to the byte-for-byte `link` rule, one substitution per record. A paper with no `pmcid` or `inPMC: N` is reported as 无 PMCID / 非 OA rather than as a failed fetch. The original wins on divergence (write both values as `原文 …；库内记录 …{{ref_n}}`), a record whose original cannot be retrieved is named in the evidence scope as a limitation, and nothing outside the route templates plus the record's own already-returned link (one attempt per record — two for the `src=1` PMC chain — only for hosts measured reachable, never a search engine, never a substitute URL, never a retry, and never a host on the blocked list) is ever fetched. Fields `paper_title`, `full_article_link`, `paper_release_time`, `journal`, `doi`, `pm_id` are citation metadata — `paper_title` / `full_article_link` go into the citation JSON byte-for-byte (except the full-text-depth link substitution above), while `paper_release_time` (a `YYYY-MM-DD HH:MM:SS` datetime string) contributes only its `YYYY-MM-DD` date part to `paper_release_time_str`; `doi` / `pm_id` / a registration id additionally serve as the route keys above, but the fields themselves never add a clinical fact. Entity fields (`projects.associate_ids` registration numbers, `trial_abbreviation`, `arms.drugs.drug_earth_id` drug IDs, `projects.company_ids` company IDs) are **display/label metadata, not clinical evidence** — they exist only to attach entity inline references to mentions already present in the report. Do not fill gaps from memory and never use technical correlation keys as evidence. Require at least two usable records: **if fewer come back, do not write a report or a citation file at all** — say in chat which selected results could not be retrieved and ask the user to re-check the selection (a one-record "comparison" is a fabricated deliverable). When some (not all) selected records are unusable or unretrieved, state that limitation in the evidence scope and keep the remaining comparison; never invent a citation entry for a record that did not return.
...
At the beginning of processing, assign stable inline markers in selection (esid) order over the records that returned: `{{ref_1}}`, `{{ref_2}}`, and so on. This numbering is only a citation label and does not establish chronology. Keep a source ledger internally with:

- citation fields from the pulled records (`paper_title`, `full_article_link`, `paper_release_time` — date part only for the release time — and when returned `journal`/`doi`/`pm_id`) for the separate final citation JSON;
