...
- **最新发布/固定回归**：用户已授权原地更新并指定永久三组输入。2026-09-18 17:05 平台记录：prompt v1.6（55,885 B / SHA `50b870badd16`）、skill v1.0.7（skill_id `ad75ec2c44334f389a0394895a47b765`），对应仓库 v0.15；回读 prompt 与 19/19 文件一致。固定组保存在 `evals/fixed-regression-inputs.json`，流程见 `docs/fixed-regression.md`；4/18/14 条共 32 唯一 ID，SQL 全命中且无需转换。R27–R29 运行中。下文“待发布”均为此前历史状态。


- **设计复核后续改动：本地待发布**。已在 Skill 正文、输入契约、抽取参考和 sys v0.15 落实登记设计字段逐项原文复核、原文优先、双来源披露、角色/阶段区别及未确认项标注。技能包 86,621 B / 19 文件，SHA `e91f474fa80b450b5b18b72acc93e9dbc12f0a7bab181a0ae77d70da994ef887`；pack 检查、diff 检查通过，deps unchanged。**下文历史 in-sync 状态不代表本次新修改已上线**；需要发布授权后在线 run 留 R 记录。测试 ID 兼容未加入 Skill。详见 [落实和验证计划](docs/test-input-resolution.md)。
