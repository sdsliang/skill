...
| `extra_esids` | array[string] |  | `None` | 临床结果的ID列表，用于精确过滤 |
...
| `selected_fields` | array[string] |  | `None` | `selected_fields` 必须是字段名字符串数组，例如：["clinical_trial_project.nct_id", "company.company_name_cn"]。 严格遵守以下规则： 1. 阅读 ALLOWED_FIELDS，通过字段描述得知其含义并选择你所需要的字段。 1. 字段名只能从 ALLOWED_FIELD_NAMES 中逐字复制，大小写必须完全一致。 2. `selected_fields` 不能为空且每项都必须是 ALLOWED_FIELD_NAMES 中的完整字符串，不能填描述文本。 ALLOWED_FIELD_NAMES: [ "clinical_r …（全文见下） |
...
| `clinical_result.extra_esid` | 临床结果的id |
