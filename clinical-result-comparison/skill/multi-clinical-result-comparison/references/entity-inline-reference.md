...
- **没有嵌套锚点**：形如 `[名字](entity:trial:[名字](entity:trial:ID))` 的双层包裹是错误（多是因为要锚的名字本身已带锚点又包了一层）。脚本里断言 `](entity:` 不出现嵌套形态，发现即失败重跑渲染，而不是事后用 `edit_file` 逐个抠。
...
3. **渲染、断言、落盘放同一个脚本里**：脚本先渲染到内存字符串 → 跑完 (a)(b)(c) 断言 → 全部通过才 `write` 到报告路径。
   这样断言失败只是**重跑一次同一脚本**（一次 `execute`），而不是回到 `edit_file` 逐条补——后者会把一次渲染失败放大成十几个工具步（相邻批次实测有过这种退化）。
