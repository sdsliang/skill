...
- 引注一侧全句不提自己的身份证（`M06` / `M15`）⇒ FAIL 不变；`arm a` 的覆盖率检查保证 `A-ATTR` 始终有牙（`M15`）。
...
- **`MUT_RUN_A` 仍指向带 seed 的 R8 副本**（见上）：`R17` 这个真·干净基线要等 4 条 R8 绑定变异重写 + `M21` 前提调整后才能接上。
...
```
arm a: flipped 40/40   (27 个变异：改主终点数值、翻转安慰剂符号、抹掉试验登记号、篡改时点、
...
arm a 假阳性回归（`NEG_A`，要求**保持绿**、退出码 0）：
                        ...
                        N26 主题式标题（无药名）
                        N27 登记号对照句（引注 ref_1 + 提对方的试验，形状取自 R12 真实产物）
...
**为什么还没换成真 run（2026-09-18 复核）**：`R17`（`20260918-095553-r17-sign-convention`）已经是「除 `O20` 外全绿」的真实产物，
天生带 `原文核对：` 行，看起来可以直接当 arm A 基线。实测把它指过去会连带打掉 4 条**绑定 R8 字节**的变异
...
也就是说换基线要先重写这几条变异（让它们按 R17 的真实措辞改），不是改一个路径常量。这件事记在台账 `O20` 条目里，
下一步单独做；在那之前 `seed_original_check` 保留（它只补一行、只补在临时副本上）。
