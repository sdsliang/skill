...
- **（2026-09-16，已落地）run v2 轮询版 + S0 键名修复**（方案 B：S0 与 run v2 同批做完；改前备份 `~/.local/state/toolsmith-publish/toolsmith-publish.v1.bak`）。
  ...
  ⑨ **P8 实验设计已定稿**（`docs/toolsmith-run-v2-polling-plan.md` §8.1，E-P8-1/2/3 + 判定表，成本 1 次新 run），**未执行**（写动作，需用户点头）；执行前应先问。剩余可选项：场景 C/D 清单、O4/O5 攒样本。
