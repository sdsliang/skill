...
2. Assign stable inline references (`{{ref_1}}`, `{{ref_2}}`, etc.) over the records that returned, in input (esid) order — a selected esid that returned no record gets no marker and no citation entry (see the unretrievable-selection rule below). Reference numbering is a presentation label, not clinical chronology. Keep the pulled `paper_title`, `full_article_link`, and `paper_release_time` only for the separate citation JSON file: `title`/`link` byte-for-byte, and `paper_release_time_str` = the `YYYY-MM-DD` date part of the `YYYY-MM-DD HH:MM:SS` field (drop the time component; never reformat further, never invent a date). These markers are machine-readable presentation tokens; do not explain or spell out the marker syntax in the user-facing evidence-scope text.
...
- **Try straight-through scripting before delegating.** When the params result is already persisted to a file under `/workspace/tool_results/…` and `execute` is available, digesting that file with a script is usually cheaper and more accurate than delegating: the raw payload never enters your context, and citation metadata stays byte-exact (a subagent summary can silently truncate it). Prefer that path; delegate only when file-based digesting is unavailable. Either way decide in one line and move on — do not spend a long reasoning block weighing delegation against direct reading. **Small selections favor direct pulls:** with 6–8 selected esids whose records are small (a few endpoints each, short `abstract_text`/`study_results`), pulling them directly in one or two `selected_fields`-bounded calls is usually faster and at least as accurate as delegating — delegation is a context-bounding device for genuinely heavy payloads, not a default for everything above 5.
...
The Markdown report file must contain only the consumer-facing synthesis and inline `{{ref_n}}` markers. Write the citation metadata separately to the fixed path `/workspace/output/citations.json` as one strict, unfenced JSON object. Each key is `ref_n`; each value contains exactly `title`, `link`, and `paper_release_time_str`: `title`/`link` copied byte-for-byte from the pulled record's `paper_title` / `full_article_link`, and `paper_release_time_str` = the date part only (`YYYY-MM-DD`) of `paper_release_time` (that field returns `YYYY-MM-DD HH:MM:SS`; drop the time component, do not reformat further, and keep an empty value empty). Only retrieved records appear here: an empty `title` means a record that never came back was cited, which is never acceptable.
...
- the citation JSON contains exactly `title`, `link`, and `paper_release_time_str` string fields per key: `title`/`link` byte-for-byte from `paper_title` / `full_article_link`, and `paper_release_time_str` equal to the `YYYY-MM-DD` date part of `paper_release_time` (no time component, no reformatting);
