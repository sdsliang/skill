...
- 修订（fragment 根因：`<header>` 命中 forbidden 子串）：Tool Smith 前端 `validateWidgetCode` 用 `lower.includes` 对整个文件做子串匹配（`<!doctype`/`<html`/`<head`/`<body`，含注释/CSS/字符串），`<header>` 因含 `<head` 前缀被误判“不是有效的 fragment”。修复：三个模板 + TRAIN-2 示例的 `<header>`/`</header>` 改为 `<div class="header">`/`</div>`，CSS `header{}` 改 `.header{}`。已用与前端相同的校验逻辑模拟验证全 OK（无 forbidden 子串、以 `<` 开头），wrapper 渲染 12 text 元素正确。该约束已同步到：`chart-templates.md`（契约新增“避免 `<head` 前缀标签”条款）、v0.8 系统提示词（120/122 行定量图与时间轴图段落均补 fragment + 子串匹配说明）、`SKILL.md`、`timeline-diagram.md`、`cross-trial-comparison.md`、三个报告模板、README。
