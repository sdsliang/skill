...
`references/input-contract.md` (*Evidence source priority*) is the single authority for the evidence boundary, retrieval permissions/budgets, analysis depth, failure reasons and citation-link substitutions. Use the selected records' clinical-content fields (`abstract_text`, `summary`, `study_results`, design/arms context), with the original source taking priority for material clinical facts. Read `abstract_text` first; for `src=1` attempt the permitted R6 → R7 PMC chain even when an abstract is present. A usable full text retrieved for the selected paper becomes its analysis surface: full-text-only facts may enter prose, tables and charts with their own time point, analysis-set and population labels. Depth alone does not determine comparability. Cite that record with C1 or C2 only after retrieving and archiving its full text; otherwise preserve its supplied link. No PMCID, not in PMC, explicit non-OA metadata and HTTP fetch failure are distinct outcomes; HTTP 500 never proves non-OA.

Keep the source-class permissions, blocked hosts, one-attempt limit (two calls for the PMC chain), whole-run cap and no-search/no-retry rules in the input contract. Record paths, depths and actual failure reasons in `原文核对：`; write same-metric, same-label divergences as `原文 …；库内记录 …{{ref_n}}`, and distinguish version differences. Citation and entity metadata are not clinical evidence; entity IDs only label mentions already supported by the selected evidence. Do not fill gaps from memory or introduce unselected studies. Require at least two usable records: **if fewer come back, do not write a report or citation file**; name the missing/unusable selections in chat and ask the user to re-check them. With at least two usable records, name any remaining gaps in the evidence scope and never invent a citation for an unretrieved record.
...
For one trial, use `templates/unified-evidence-report.md`; for multiple trials use `templates/cross-trial-report.md` (or `templates/mixed-comparison-report.md` for a mixed set), per the routing above. A one-trial report must read as one trial-level evidence synthesis. Use a timeline table with one row per evidence state. Use endpoint tables that show the earliest and latest distinct values, analysis status, maturity, and inline markers. The separate citation JSON is the only source listing. Draft the complete report, then write it to `/workspace/output/report.md` and the citation JSON to `/workspace/output/citations.json` per `references/file-delivery.md` (do not stream the report into the chat body).
